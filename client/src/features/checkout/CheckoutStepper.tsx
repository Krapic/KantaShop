...
const steps = ['Adresa', 'Plaćanje', 'Pregled'];
...
export default function CheckoutStepper() {
    ...
    const confirmPayment = async () => {
        ...
        try {
            if (!confirmationToken || !basket?.clientSecret)
                throw new Error('Nije moguće obraditi plaćanje');
    ...
    const createOrderModel = async () => {
        ...
        if (!shippingAddress || !paymentSummary) throw new Error('Problem sa stvaranjem narudžbe');
    ...
    if (isLoading) return <Typography variant="h6">Učitavanje kupnje...</Typography>
    ...
    return (
        <Paper sx={{ p: 3, borderRadius: 3 }}>
            ...
            <Box sx={{ mt: 2 }}>
                <Box sx={{ display: activeStep === 0 ? 'block' : 'none' }}>
                    ...
                    <FormControlLabel
                        ...
                        />}
                        label='Spremi kao zadanu adresu'
                    />
                </Box>
            ...
            <Box display='flex' paddingTop={2} justifyContent='space-between'>
                <Button onClick={handleBack}>Povratak</Button>
                <LoadingButton
                    onClick={handleNext}
                    disabled={
                    ...
                    loading={submitting}
                >
                    {activeStep === steps.length - 1 ? `Pay ${currencyFormat(total)}` : 'Nastavak'}
                </LoadingButton>
            </Box>
        </Paper>
    )
